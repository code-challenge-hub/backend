...
public class CustomLogoutFilter extends GenericFilterBean {
    ...
    public CustomLogoutFilter(JWTUtil jwtUtil, RefreshTokenUtil refreshTokenUtil, String filterProcessesUrl) {
    ...
    private void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (!requestUri.matches("^"+filterProcessesUrl.replace("/", "\\/")+"$")) {
        ...
        Boolean isExist = refreshTokenUtil.existRefreshToken(email);
        if (!isExist) {
            //response status code
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            return;
        ...
        response.addCookie(refreshTokenUtil.cleCookie());
